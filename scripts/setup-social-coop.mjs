...
const sql = `
-- ============================================================
-- SEARCH PROFILES (SECURITY DEFINER — bypass RLS)
-- Permite buscar usuários por display_name sem RLS bloquear
-- ============================================================
...
-- ============================================================
-- CO-OP MISSIONS — tabela de missões em conjunto
-- ============================================================
...
-- ============================================================
-- RPC: create_co_op_mission — cria missão e já adiciona criador
-- ============================================================
...
-- ============================================================
-- RPC: complete_co_op_mission — marca membro como completo e concede XP
-- ============================================================
...
BEGIN
  ...
  SELECT xp_claimed, xp_per_player INTO v_claimed, v_xp
  ...
  -- Marca membro como completo + concede XP
  ...
  -- Se todos concluíram, fecha a missão
...
const req = https.request({
  ...
  res.on('end', () => {
    try {
      ...
      else console.log('✅ search_profiles RPC + co_op_missions tabelas criadas');
    } catch { console.log(d); }
  });
});
req.write(body);
