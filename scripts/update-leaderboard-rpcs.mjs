...
// Reescreve as 6 funções de leaderboard para incluir current_class_name via LEFT JOIN
const sql = `
...
LANGUAGE sql SECURITY DEFINER STABLE AS $$
  ...
  FROM public.profiles p
  LEFT JOIN public.classes c ON c.id = p.current_class_id
  WHERE p.region = p_region AND p.starter_class = p_class
  ORDER BY p.total_xp DESC
  LIMIT p_limit;
$$;
...
const req = https.request({
  ...
  res.on('end', () => {
    try {
      ...
      else console.log('✅ Funções de leaderboard atualizadas com current_class_name');
    } catch { console.log(d); }
  });
});
req.write(body);
