-- =============================================================
-- Migration: Cria tabelas sociais ausentes no backend ativo
-- Tabelas: friend_requests, adventure_journal, achievements,
--          user_achievements, npc_challenge_completions
-- Todas com CREATE TABLE IF NOT EXISTS + RLS
-- =============================================================
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    WHERE tablename = 'friend_requests' AND policyname = 'users_see_own_requests'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE tablename = 'friend_requests' AND policyname = 'users_send_requests'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE tablename = 'friend_requests' AND policyname = 'receiver_updates_status'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE tablename = 'friend_requests' AND policyname = 'users_delete_own_requests'
  ) THEN
...
CREATE TABLE IF NOT EXISTS public.adventure_journal (
  ...
  mood        text        CHECK (mood IN ('feliz', 'neutro', 'cansado', 'motivado', 'ansioso'))
                            DEFAULT 'neutro',
  created_at  timestamptz NOT NULL DEFAULT now(),
  updated_at  timestamptz NOT NULL DEFAULT now(),
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE tablename = 'adventure_journal' AND policyname = 'users_own_journal'
  ) THEN
...
INSERT INTO public.achievements
  (slug, title, description, icon, xp_reward, gold_reward, condition_type, condition_value)
VALUES
...
-- ─────────────────────────────────────────
-- 4. CONQUISTAS DO USUÁRIO (user_achievements)
-- ─────────────────────────────────────────
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE tablename = 'user_achievements' AND policyname = 'users_see_own_achievements'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE tablename = 'user_achievements' AND policyname = 'system_grants_achievements'
  ) THEN
...
-- ─────────────────────────────────────────
-- 5. DESAFIOS DE NPC (npc_challenge_completions)
-- ─────────────────────────────────────────
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE tablename = 'npc_challenge_completions' AND policyname = 'users_own_npc_completions'
  ) THEN
