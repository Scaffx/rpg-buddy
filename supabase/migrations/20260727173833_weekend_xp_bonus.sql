CREATE OR REPLACE FUNCTION public._weekend_xp_bonus(
...
AS $$
  SELECT CASE
    WHEN p_frequency_type = 'daily'
      AND EXTRACT(dow FROM p_today)::integer = 0
      AND COALESCE(p_days_of_week, '[]'::jsonb) ? 'Dom'
      THEN 0.25
    WHEN p_frequency_type = 'daily'
      AND EXTRACT(dow FROM p_today)::integer = 6
      AND (
        COALESCE(p_days_of_week, '[]'::jsonb) ? 'Sáb'
        OR COALESCE(p_days_of_week, '[]'::jsonb) ? 'Sab'
      )
      THEN 0.25
    ELSE 0
  END;
...
REVOKE ALL ON FUNCTION public._weekend_xp_bonus(text, jsonb, date)
  FROM PUBLIC, anon, authenticated;

-- Bônus de fim de semana: +25% aditivo somente para recorrentes diárias
-- agendadas para Sáb/Dom e concluídas no próprio dia.
...
-- Preserva o dispatcher diário/semanal atual e normaliza weekend_bonus=false
-- para one-shot e frequência flexível.
ALTER FUNCTION public.complete_mission(uuid)
  RENAME TO _complete_mission_with_weekly;
...
BEGIN
  ...
  v_result := public._complete_mission_with_weekly(p_mission_id);

  RETURN v_result || jsonb_build_object(
    'weekend_bonus',
    COALESCE((v_result->>'weekend_bonus')::boolean, false)
  );
END;
$wrapper$;
