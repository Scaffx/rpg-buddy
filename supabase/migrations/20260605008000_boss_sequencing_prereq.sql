-- Roadmap #1: Sequenciamento de bosses (ordem obrigatória).
-- Cada boss exige a derrota do "boss anterior elegível" na escada (por nível).
-- Bosses condicionais que podem escapar/fugir NÃO podem ser pré-requisito de
-- ninguém (senão criam soft-lock): Fênix Renascente (escapa sem item),
-- Guerreiro Imortal (foge sem Cabeça de Basilisco) e o combo "Fênix + Esfinge
-- do Deserto" (será removido no roadmap #2). Eles ainda PODEM ter pré-requisito
-- (são alcançáveis normalmente), só não bloqueiam o próximo da escada.

ALTER TABLE public.bosses
  ADD COLUMN IF NOT EXISTS prereq_boss_id uuid
  REFERENCES public.bosses(id) ON DELETE SET NULL;

COMMENT ON COLUMN public.bosses.prereq_boss_id IS
  'Boss que precisa ser derrotado antes deste (cadeia de progressão). NULL = sem pré-requisito.';

-- Conjunto de nomes "não-bloqueantes" (condicionais/escape).
-- Derivação re-executável: prereq = boss elegível imediatamente anterior por (level, name).
...
-- Gate server-side no resolve de combate (defense-in-depth): mesmo que o client
-- seja burlado, a recompensa só é creditada se o pré-requisito estiver derrotado.
CREATE OR REPLACE FUNCTION public.resolve_boss_battle(p_boss_id uuid, p_won boolean, p_damage integer DEFAULT 0)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- 🔒 Pré-requisito de sequência: precisa ter derrotado o boss anterior da cadeia.
  IF v_boss.prereq_boss_id IS NOT NULL AND NOT EXISTS (
    SELECT 1 FROM public.boss_battles
    WHERE user_id = v_uid AND boss_id = v_boss.prereq_boss_id AND won = true
  ) THEN
  ...
  -- Consome as chaves (não há devolução em caso de derrota)
