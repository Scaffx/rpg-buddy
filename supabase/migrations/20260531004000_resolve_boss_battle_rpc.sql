-- ================================================================
-- resolve_boss_battle(p_boss_id, p_won, p_damage) — recompensa de boss
-- server-side. Valida chaves + "já derrotado", consome chaves/buffs e
-- credita XP/ouro lendo os valores do boss no banco (não do client).
-- O resultado de vitória/dano ainda vem do combate client-side; o combate
-- 100% autoritativo virá com a reformulação de combate.
-- ================================================================

CREATE OR REPLACE FUNCTION public._consume_one_shot_buff(p_uid uuid, p_effects text[])
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  UPDATE public.user_buffs ub SET active = false
   WHERE ub.id = (
     ...
     JOIN public.shop_items si ON si.id = ub2.item_id
     WHERE ub2.user_id = p_uid AND ub2.active = true
       AND (ub2.expires_at IS NULL OR ub2.expires_at > now())
       AND si.effect = ANY(p_effects)
...
DECLARE
  ...
  v_cur_keys int;
  v_new_total int; v_new_level int; v_refund int;
...
BEGIN
  ...
  -- Consome buffs de uso único de combate
  ...
  ELSE
    v_refund := v_keys_cost / 2;
    IF v_refund > 0 THEN
      UPDATE public.profiles SET boss_keys = (v_cur_keys - v_keys_cost) + v_refund WHERE user_id = v_uid;
    END IF;
    INSERT INTO public.activity_log (user_id, action, description, xp_gained)
    VALUES (v_uid, 'boss_failed',
            'Derrota contra o boss. Dano causado: ' || v_damage || '.' ||
            CASE WHEN v_refund > 0 THEN ' ' || v_refund || ' chave(s) devolvidas.' ELSE '' END, 0);
    RETURN jsonb_build_object('won', false, 'xp_gained', 0, 'gold_gained', 0, 'damage', v_damage, 'refunded_keys', v_refund);
  END IF;
END;
$$;

REVOKE EXECUTE ON FUNCTION public.resolve_boss_battle(uuid, boolean, integer) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.resolve_boss_battle(uuid, boolean, integer) FROM anon;
GRANT  EXECUTE ON FUNCTION public.resolve_boss_battle(uuid, boolean, integer) TO authenticated;
