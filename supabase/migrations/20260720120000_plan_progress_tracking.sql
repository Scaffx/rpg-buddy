-- ================================================================
-- Progresso de Prioridades (plans.current_value) — Torneira.
--
-- BUG: a barra de progresso de um plano ficava 0/target para sempre.
-- CAUSA RAIZ: a coluna public.plans.current_value nunca existiu no schema.
-- Por isso o bloco de "planos vinculados" foi removido do complete_mission
-- ("progresso de planos precisa de correção de schema à parte"), e o frontend
-- lê plan.current_value (undefined -> 0).
--
-- FIX (isolado, sem tocar no fluxo de save de planos):
--   1. Adiciona a coluna plans.current_value (int, default 0).
--   2. Backfill: reconstrói o progresso das prioridades existentes a partir
--      das conclusões históricas (desde a criação do plano), com teto por
--      missão (value_per_completion) e teto por plano (target_value).
--   3. complete_mission: cada conclusão de missão vinculada avança +1 em todo
--      plano do usuário que a inclui (teto no alvo).
--   4. undo_mission: reverte -1 (piso 0).
--
-- Semântica (spec "Rotina é a Torneira" + UI): value_per_completion = número
-- de conclusões necessárias daquela missão; target_value = soma; cada
-- conclusão de uma missão vinculada = +1 de progresso.
-- ================================================================
...
-- 2. Backfill de progresso das prioridades existentes ---------------------
WITH pm_progress AS (
  SELECT
    pm.plan_id,
    LEAST(
      GREATEST(1, pm.value_per_completion),
      CASE
        WHEN COALESCE(jsonb_array_length(COALESCE(m.days_of_week, '[]'::jsonb)), 0) > 0 THEN
          -- missão diária: conta datas concluídas desde a criação do plano
          (SELECT count(DISTINCT mdc.completion_date)
             FROM public.mission_daily_completions mdc
            WHERE mdc.mission_id = pm.mission_id
              AND mdc.completion_date >= p.created_at::date)
        ELSE
          -- missão única: 1 se concluída após a criação do plano
          CASE WHEN COALESCE(m.completed, false)
                    AND (m.completed_at IS NULL OR m.completed_at >= p.created_at)
               THEN 1 ELSE 0 END
      END
    ) AS done
...
UPDATE public.plans pl
   SET current_value = LEAST(pl.target_value, agg.total_done)
  FROM (
    SELECT plan_id, COALESCE(SUM(done), 0)::int AS total_done
    FROM pm_progress
    GROUP BY plan_id
  ) agg
 WHERE agg.plan_id = pl.id;

-- 3. complete_mission — reintroduz o bloco de progresso de planos ---------
...
BEGIN
  ...
  -- ── Progresso de planos vinculados (Torneira: a rotina alimenta a meta) ──
  -- Cada conclusão de uma missão vinculada avança +1 em todo plano do usuário
  -- que a inclui, com teto no alvo do plano. (Reintroduzido junto com a coluna
  -- plans.current_value, cuja ausência havia matado este bloco.)
  UPDATE public.plans pl
     SET current_value = LEAST(pl.target_value, COALESCE(pl.current_value, 0) + 1),
         updated_at = now()
   WHERE pl.user_id = v_uid
     AND EXISTS (
       SELECT 1 FROM public.plan_missions pm
       WHERE pm.plan_id = pl.id AND pm.mission_id = p_mission_id
     );
...
-- 4. undo_mission — reverte o progresso de planos vinculados ---------------
...
BEGIN
  ...
  -- Reverte progresso de planos vinculados (-1, piso 0).
  UPDATE public.plans pl
     SET current_value = GREATEST(0, COALESCE(pl.current_value, 0) - 1),
         updated_at = now()
   WHERE pl.user_id = v_uid
     AND EXISTS (
       SELECT 1 FROM public.plan_missions pm
