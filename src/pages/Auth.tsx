...
export default function Auth() {
  ...
  const handleSaveNewPassword = async (e: React.FormEvent) => {
    ...
    if (newPassword.length < 6) {
      toast({ title: t('app.auth.toast_password_too_short_title'), description: t('app.auth.toast_password_too_short_desc'), variant: 'destructive' });
      return;
    }
    ...
    try {
      ...
      toast({ title: t('app.auth.toast_password_updated_title'), description: t('app.auth.toast_password_updated_desc') });
      // Limpa o hash e redireciona para o app
      window.history.replaceState(null, '', window.location.pathname);
      navigate('/');
    } catch (err: any) {
      toast({ title: t('app.auth.toast_save_password_error'), description: err.message, variant: 'destructive' });
    } finally {
      setSavingPassword(false);
    }
  };

  // Usa o código estável do AuthError (não a string, que muda com a localização do Supabase).
  const isEmailNotConfirmed = (err: any) =>
    err?.code === 'email_not_confirmed' ||
    (typeof err?.message === 'string' && err.message.toLowerCase().includes('email not confirmed'));

  const isInvalidCredentials = (err: any) =>
    err?.code === 'invalid_credentials' ||
    (typeof err?.message === 'string' && err.message.toLowerCase().includes('invalid login credentials'));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      } else if (isLogin && isInvalidCredentials(err)) {
        toast({
          title: t('app.auth.toast_invalid_credentials_title'),
          description: t('app.auth.toast_invalid_credentials_desc'),
          variant: 'destructive',
        });
      } else {
        toast({
            ...
            {isPasswordReset ? (
              <form onSubmit={handleSaveNewPassword} className="space-y-4">
                <div className="flex items-center gap-2 mb-4 text-primary">
                  ...
                  <h2 className="font-semibold text-lg">{t('app.auth.new_password_title')}</h2>
                </div>
                <div>
                  <label className="text-sm font-medium text-foreground mb-1.5 block">
                    {t('app.auth.new_password_label')}
                  </label>
                  <Input
                    type="password"
                    value={newPassword}
                ...
                >
                  ...
                  {t('app.auth.button_save_password')}
                </Button>
              </form>
            ) : (
              <>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="relative my-2">
                      ...
                      <div className="relative flex justify-center text-xs uppercase">
                        <span className="bg-card px-2 text-muted-foreground">{t('app.auth.divider_or')}</span>
                      </div>
                    </div>
                    ...
                    <Button
                      ...
                      variant="outline"
                      className="w-full border-discord/50 hover:border-discord hover:bg-discord/10 font-semibold gap-2 disabled:opacity-60"
                    >
                      {discordLoading && <Loader2 className="w-4 h-4 animate-spin" />}
                      <svg className="w-5 h-5 text-discord" viewBox="0 0 24 24" fill="currentColor">
