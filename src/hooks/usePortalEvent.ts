...
export type PortalEvent = {
  ...
  ends_at: string;
  hours_left: number;
  /** Cor do portal — null se não escaneou e nunca entrou */
  portal_color: PortalColor | null;
  /** true se a cor já foi revelada (escaneou ou fechou) */
  color_revealed: boolean;
  /** true se este usuário já fechou o portal hoje */
  already_completed: boolean;
  /** quantos jogadores já fecharam o portal hoje */
  participant_count: number;
  runs_this_week: PortalRunRecord[];
  /** dungeon sorteada aguardando o usuário */
  pending_dungeon: FragmentTier | null;
...
export const PORTAL_COLORS: Record<PortalColor, {
  ...
  /** % de chance de cair fragmentos */
  fragmentChance: number;
  /** máximo de fragmentos por drop */
  maxFragments: number;
...
/** Escaneia o portal usando 1x "Escaner de Portal" do inventário */
export function useScanPortal() {
  ...
  return useMutation({
    mutationFn: async (params: {
      ...
      return data as {
        ...
        frags_dropped: number;
        frag_per_player: number;
        participant_count: number;
        legendary_item: boolean;
        dungeon_tier: FragmentTier;
        error?: string;
      };
    },
    onSuccess: () => {
...
/** Confirma entrada na dungeon sorteada — debita 10 fragmentos */
export function useClaimPendingDungeon() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async () => {
